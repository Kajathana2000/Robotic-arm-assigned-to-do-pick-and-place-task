% Define the DH parameters for the ABB IRB 1100-4/0.58 (approximate values)
L(1) = Link('d', 330, 'a', 70,   'alpha', -pi/2);  % Joint 1
L(2) = Link('d', 0,   'a', 300,  'alpha', 0);      % Joint 2
L(3) = Link('d', 0,   'a', 40,   'alpha', -pi/2);  % Joint 3
L(4) = Link('d', 320, 'a', 0,    'alpha', pi/2);   % Joint 4
L(5) = Link('d', 0,   'a', 0,    'alpha', -pi/2);  % Joint 5
L(6) = Link('d', 72,  'a', 0,    'alpha', 0);      % Joint 6

% Create the robot model
IRB1100 = SerialLink(L, 'name', 'ABB IRB 1100-4/0.58');
...
for i = 1:length(targets)
    ...
    q_solution = IRB1100.ikine(targets{i}, 'mask', [1 1 1 0 0 0]);
    ...
    plot_robot(IRB1100, q_solution, target_names{i});

    % Store the end-effector position for 3D trajectory plotting
    T_computed = IRB1100.fkine(q_solution);
...
ylabel('Y (mm)');
zlabel('Z (mm)');
title('3D Trajectory of the ABB IRB 1100-4/0.58 End-Effector');
...
function plot_robot(robot, q_solution, target_name)
    ...
    axis([-1000 1000 -1000 1000 0 1000]); 
    title(['ABB IRB 1100-4/0.58 Robot Arm - ', target_name]);
end
